...
def step(t, state, liveAgents):
    ...
    for key in liveAgents.copy():
        if key in liveAgents:
            ...
            #Posar la seguent linia?
           # agent.act(t, state, liveAgents, age_wolf)
...
speed_bunny_max = 5 # maximum bunny speed (for natural selection study)
speed_bunny_min = 5  # minimum bunny speed (for natural selection study)
visibility_bunny = 10  # vision range of bunnies
gestChance_bunny = 0.0025# chance to want to reproduce for bunnies (initial value 0.0008)
# reproduction status (0 for don't want to reproduce, 1 elsewise)
gestStatus_bunny = 0
gestNumber_bunny = 4  # bunnies created per reproduction
...
visibility_fox = 200 # vision range of foxes
...
hungerReward_fox = 300  # hunger reward per bunny kill (original 150)
maxHunger_fox = 500  # hunger max limit (original 500)
gestChance_fox = 0.0004  # chance to want to reproduce for foxes (original 0.0004)
...
speed_wolf = 2# fox speed
visibility_wolf = 50  # vision range of foxes
age_wolf = 100000000  # fox age
...
hungerReward_wolf = 500  # hunger reward per FOX kill
maxHunger_wolf = 1000 # hunger max limit
gestChance_wolf = 0.0002  # chance to want to reproduce for foxes
# reproduction status for bunnies (0 for don't want to reproduce, 1 elsewise)
gestStatus_wolf = 0
gestNumber_wolf = 1  # foxes created per reproduction



# Setting up the plots
#fig = plt.figure()
#ax1 = plt.subplot(221, title="Ecosystem (blue=bunny; red=fox; black=wolf)",
                  #xlabel="x (-)", ylabel="y (-)")
#plt.xlim(0, w)
#plt.ylim(0, h)
#bunnies, = ax1.plot([], [], 'bo', ms=3)
#foxes, = ax1.plot([], [], 'ro', ms=3)
#wolves, = ax1.plot([], [], 'go', ms=3)
...
#llista per la memòria de ratios
bunny_average_list=[]
fox_average_list=[]
wolf_average_list=[]
...
for o in range(0, iterations):
    ...
    (state, liveAgents) = createWorld(w, h, n_bunnies,
                                      speed_bunny_min, speed_bunny_max, visibility_bunny, gestChance_bunny,
                                      gestStatus_bunny,
                                      gestNumber_bunny, age_bunny, n_foxes, speed_fox, visibility_fox, age_fox,
                                      huntStatus_fox,
                                      hunger_fox, hungerThresMin_fox, hungerThresMax_fox, hungerReward_fox,
                                      maxHunger_fox,
                                      gestChance_fox, gestStatus_fox, gestNumber_fox, n_wolves, speed_wolf,
                                      visibility_wolf, huntStatus_wolf, age_wolf,
                                      hunger_wolf, hungerThresMin_wolf, hungerThresMax_wolf, hungerReward_wolf,
                                      maxHunger_wolf, gestChance_wolf,
                                      gestStatus_wolf, gestNumber_wolf)
    t = 0  # time
    T = []
    popBunnyList = []
    ...
    for t in range(0, max_t):
        ...
        if popBunnyList[t-1] ==0:
            counter_fail += 1
            break
        elif popFoxList[t-1] ==0:
            counter_fail += 1
            break
        elif popWolfList[t-1] ==0:
            counter_fail += 1
            break
        elif popBunnyList[t-1] >200:
            counter_fail += 1
            break
        elif popFoxList[t-1] >200:
            counter_fail += 1
            break
        elif popWolfList[t-1] >200:
            counter_fail += 1
            break
...
#if popBunnyList[t-1]!= 0 and popFoxList[t-1]!=0 and popWolfList[t-1]!=0 and popBunnyList[t-1]<200 and popFoxList[t-1]<200 and popWolfList[t-1]<200:
    #Posant la condició de success es podria triar entre la situacio en que sobreviuen tots i en les que no.
    #Fem la mitjana dels ratios dividint per les iteracions, que en aquest cas es diuen "o"
    ratio_bunny = (popBunnyList[t-1]/n_bunnies)
    ratio_fox = (popFoxList[t-1]/n_foxes)
    ratio_wolf = popWolfList[t-1]/n_wolves

    iterations_list.append(o+1)
    ratio_bunny_list.append(ratio_bunny)
    ratio_fox_list.append(ratio_fox)
    ratio_wolf_list.append(ratio_wolf)

    ratio_bunny_average=0
    ratio_fox_average =0
    ratio_wolf_average = 0
    for k in range(0,o+1):

        ratio_bunny_average += ratio_bunny_list[k]/(o+1)
        ratio_fox_average += ratio_fox_list[k] / (o + 1)
        ratio_wolf_average += ratio_wolf_list[k] / (o + 1)
    #print(ratio_bunny_list[0])

    bunny_average_list.append(ratio_bunny_average)
    fox_average_list.append(ratio_fox_average)
    wolf_average_list.append(ratio_wolf_average)




print(iterations_list)
print(bunny_average_list)
print(fox_average_list)

# plotting stuff:
plt.title("Reproduction study for t=1600")
plt.xlabel("iterations")
plt.ylabel("ratio (final pop / initial pop)")

#plt.plot(iterations_list, ratio_bunny_list,'bo',label="Bunnies")
#plt.plot(iterations_list, ratio_fox_list, 'ro', label="Foxes")
#plt.plot(iterations_list, ratio_wolf_list,'go',label="Wolves")

plt.plot(iterations_list, bunny_average_list,'bo',label="Bunnies")
plt.plot(iterations_list, fox_average_list, 'ro', label="Foxes")
plt.plot(iterations_list, wolf_average_list,'go',label="Wolves")
